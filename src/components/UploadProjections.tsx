...
export function UploadProjections() {
  ...
  const handleCreateSemester = async () => {
    if (!newSemesterTerm || newSemesterTerm.length !== 6) {
      setModal({ isOpen: true, type: 'error', title: 'Error', message: 'El término (TERM) debe tener 6 dígitos. Ej: 202625' });
      return;
    }
    const yearStr = newSemesterTerm.slice(0, 4);
    const period = newSemesterTerm.slice(4, 6);
    const year = parseInt(yearStr);

    if (!['15', '20', '25', '30'].includes(period)) {
      setModal({ isOpen: true, type: 'error', title: 'Error', message: 'El término debe terminar en 15, 20, 25 o 30.' });
      return;
    }

    let codeStr = '';
    if (period === '15') codeStr = `Sem Sep/Ene ${year - 1}-${year.toString().slice(-2)} PR`;
    if (period === '20') codeStr = `Intensivo Feb ${year} PR`;
    if (period === '25') codeStr = `Sem Mar/Jul ${year - 1}-${year.toString().slice(-2)} PR`;
    if (period === '30') codeStr = `Intensivo Ago ${year} PR`;

    const semName = `${newSemesterTerm} ${codeStr}`;

    const { error } = await supabase.from('semestre').insert({
      sem_nombre: semName,
    ...
    if (error) {
      setModal({ isOpen: true, type: 'error', title: 'Error al crear', message: error.message });
    } else {
      setModal({ isOpen: true, type: 'success', title: 'Éxito', message: 'Semestre creado correctamente.' });
      setNewSemesterTerm('');
      fetchSemesters();
    }
  };

  const handleSetActiveSemester = async (semId: number) => {
    // Primero, desactivar todos
    await supabase.from('semestre').update({ sem_is_active: false }).not('sem_id', 'is', null);
    // Activar el seleccionado
    const { error } = await supabase.from('semestre').update({ sem_is_active: true }).eq('sem_id', semId);
    ...
    <div className="flex flex-col h-full bg-slate-50 dark:bg-slate-900 overflow-y-auto">
      ...
      <main className="flex-1 p-8">
        <div className="max-w-2xl mx-auto bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 p-8">
          ...
          <div className="mb-8">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-4 gap-3">
              ...
              <button
                type="button"
                onClick={() => {
                  setUploadType('semestres');
                  resetPreview();
                }}
                disabled={loading}
                className={`px-4 py-3 rounded-lg border text-sm font-semibold transition-colors ${uploadType === 'semestres'
                  ? 'border-primary/40 bg-primary/10 text-primary'
                  : 'border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700/50'
                  }`}
              >
          ...
          {uploadType === 'semestres' ? (
            <div className="mb-8">
              <h3 className="text-lg font-bold text-slate-800 dark:text-white mb-4">Gestión de Semestres</h3>

              <div className="flex gap-2 mb-6">
                <input
                  type="text"
                  placeholder="Ej: 202625"
                  className="flex-1 rounded-lg border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 text-slate-900 dark:text-white"
                  value={newSemesterTerm}
                  onChange={(e) => setNewSemesterTerm(e.target.value.replace(/\D/g, '').slice(0, 6))}
                />
                <button
                ...
                <table className="w-full text-left text-sm text-slate-600 dark:text-slate-300">
                  ...
                  <tbody>
                    {semestersList.map(sem => (
                      <tr key={sem.sem_id} className="border-b border-slate-100 dark:border-slate-700/50 last:border-0 hover:bg-slate-50 dark:hover:bg-slate-700/20">
                        <td className="px-4 py-3 font-medium text-slate-900 dark:text-white">{sem.TERM}</td>
                        <td className="px-4 py-3">{sem.sem_nombre}</td>
                        ...
                        <td className="px-4 py-3 text-right">
                          {!sem.sem_is_active && (
                            <button
                              onClick={() => handleSetActiveSemester(sem.sem_id)}
            ...
            <>
              {(uploadType === 'proyecciones' || uploadType === 'horarios') && (
                <div className="mb-6">
                  ...
                  <select
                    ...
                    disabled={loading || previewData.length > 0 || schedulePreview.length > 0}
                  ...
                  >
                    ...
                    {semestersList.map(sem => (
                      <option key={sem.sem_id} value={sem.TERM}>
                        {sem.sem_nombre} {sem.sem_is_active ? '(Activo)' : ''}
                      </option>
                    ))}
                  </select>
                  {uploadType === 'proyecciones' && (
                    <p className="text-xs text-slate-500 mt-2">El código TERM debe coincidir con el nombre de la hoja en el Excel.</p>
                  )}
                </div>
              )}
