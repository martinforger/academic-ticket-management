...
export const ForgotPasswordPage: React.FC<ForgotPasswordPageProps> = ({ onBackToLogin }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // For reset password, we want to redirect back to the app
    // The URL should be where your app is hosted
      ...
      <main className="flex-1 tech-grid relative flex items-center justify-center px-4 py-8 z-10">
        <div className="w-full max-w-md glass-morphism dark:glass-morphism rounded-3xl shadow-[0_20px_50px_rgba(0,0,0,0.1)] border border-white/50 dark:border-slate-800/50 p-8 @container animate-scaleIn relative overflow-hidden group">
          ...
          <div className="text-center mb-8 animate-fadeInUp animate-delay-100">
            ...
            <h2 className="text-2xl font-black tracking-tight text-[#0d141b] dark:text-white mb-1">Recuperar Contraseña</h2>
            <p className="text-[#4c739a] dark:text-slate-400 text-sm">Te enviaremos un enlace para restablecerla</p>
          </div>
          ...
          {!success ? (
            <form className="space-y-5" onSubmit={handleSubmit}>
              ...
              <button
                disabled={loading}
                className="w-full bg-primary hover:bg-primary/90 disabled:opacity-50 text-white font-black py-3.5 px-6 rounded-2xl transition-all shadow-xl shadow-primary/25 flex items-center justify-center gap-3 active:scale-[0.98] animate-fadeInUp animate-delay-300 hover-glow"
                type="submit"
              >
                {loading ? (
            ...
            <div className="text-center space-y-6 animate-fadeInUp">
              <div className="p-4 rounded-xl bg-green-50 dark:bg-green-900/20 text-green-600 dark:text-green-400 text-sm font-medium border border-green-100 dark:border-green-900/30">
          ...
          {!success && (
            <div className="mt-8 pt-6 border-t border-slate-200/50 dark:border-slate-700/50 text-center animate-fadeInUp animate-delay-400">
              <button onClick={onBackToLogin} className="text-primary font-black hover:underline">
